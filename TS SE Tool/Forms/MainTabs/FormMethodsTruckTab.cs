...
namespace TS_SE_Tool
{
    public partial class FormMain
    {
        ...
        private void SelectSavedTruck()
        {
            List<string> savedTruckLinks = new List<string>();
            string assignedVehicleLink = SiiNunitData.Player.assigned_vehicles;

            if (!String.IsNullOrEmpty(assignedVehicleLink) && assignedVehicleLink != "null" &&
                SiiNunitData.SiiNitems.TryGetValue(assignedVehicleLink, out dynamic assignedVehicleItem) &&
                assignedVehicleItem is Save.Items.Player_Vehicles assignedVehicle &&
                !String.IsNullOrEmpty(assignedVehicle.vehicle) && assignedVehicle.vehicle != "null")
            {
                savedTruckLinks.Add(assignedVehicle.vehicle);
            }

            savedTruckLinks.Add(SiiNunitData.Player.my_truck);
            savedTruckLinks.Add(SiiNunitData.Player.assigned_truck);

            foreach (string savedTruckLink in savedTruckLinks)
            {
                if (String.IsNullOrEmpty(savedTruckLink) || savedTruckLink == "null")
                    continue;

                for (int rowIndex = 0; rowIndex < comboBoxUserTruckCompanyTrucks.Items.Count; rowIndex++)
                {
                    DataRowView row = comboBoxUserTruckCompanyTrucks.Items[rowIndex] as DataRowView;

                    if (row != null && row["UserTruckNameless"].ToString() == savedTruckLink)
                    {
                        comboBoxUserTruckCompanyTrucks.SelectedIndex = rowIndex;
                        return;
                    }
                }
            }
