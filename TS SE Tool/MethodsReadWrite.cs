...
namespace TS_SE_Tool
{
    public partial class FormMain : Form
    {
        ...
        private void LoadSaveFile(object sender, DoWorkEventArgs e)
        {
            ...
            {
                SaveFileFormat = GetSaveFileFormat(SiiSavePath).saveFileFormat;
            ...
            //=== Save info
            InfoFileFormat = GetSaveFileFormat(SiiInfoPath).saveFileFormat;
        ...
        private void NewWrireSaveFile(object sender, DoWorkEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string infoText = MainSaveFileInfoData.PrintOut();
                    if (InfoFileFormat == 2)
                        File.WriteAllBytes(SiiInfoPath, EncodeScsC(Encoding.UTF8.GetBytes(infoText), File.ReadAllBytes(SiiInfoPath)));
                    else if (InfoFileFormat == 1)
                        File.WriteAllText(SiiInfoPath, infoText, Encoding.UTF8);
                    else
                        throw new InvalidDataException("Unsupported info.sii format: " + InfoFileFormat);
                }

                //Write Save data
                string saveText = SiiNunitData.PrintOut(MainSaveFileInfoData.Version);
                if (SaveFileFormat == 2)
                    File.WriteAllBytes(SiiSavePath, EncodeScsC(Encoding.UTF8.GetBytes(saveText), File.ReadAllBytes(SiiSavePath)));
                else if (SaveFileFormat == 1)
                    File.WriteAllText(SiiSavePath, saveText, Encoding.UTF8);
                else
                    throw new InvalidDataException("Unsupported game.sii format: " + SaveFileFormat);
