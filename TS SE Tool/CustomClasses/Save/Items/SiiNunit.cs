...
namespace TS_SE_Tool.Save.Items
{
    class SiiNunit
    {
        ...
        internal List<string> PlayerVehicles = new List<string>();
        ...
        public dynamic DetectTag(string nameless, string tagLine, string[] _input)
        {
            switch (tagLine)
            {
                ...
                case "player_vehicles":
                    {
                        PlayerVehicles.Add(nameless);
                        return new Player_Vehicles(_input);
                    }
        ...
        internal string PrintOut(uint _version)
        {
            ...
            SiiNBlockCore.WrittenBlocks.Clear();
            ...
            foreach (string item in PlayerVehicles)
            {
                returnSB.AppendLine(((Player_Vehicles)SiiNitems[item]).PrintOut(_version, item));
            }
            ...
            if (!string.IsNullOrEmpty(Economy.registry) && SiiNitems.ContainsKey(Economy.registry))
            ...
            if (!string.IsNullOrEmpty(Economy.bus_job_log) && SiiNitems.ContainsKey(Economy.bus_job_log))
            {
            ...
            if (NamelessControlList.Count > 0)
            {
                ...
                foreach (string item in skippedBlocks.Where(x => x != null && x != "null" && !SiiNBlockCore.WrittenBlocks.Contains(x)))
                {
                    returnSB.AppendLine(SiiNitems[item].PrintOut(_version, item));
                }
            }
